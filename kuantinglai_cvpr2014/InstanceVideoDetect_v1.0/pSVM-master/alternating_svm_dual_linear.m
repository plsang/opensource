function [model] = alternating_svm_dual_linear(K, Bag_idx, Bag_prop, para, data)
...
%% initialize
% if length(Bag_prop) == length(Bag_idx) % simply initialize as the given labels if only one instance per bag
...
%     end
    model_svm = regular_svm_linear_wrapper(K, model.y, para);
...
function [objective, obj_1, obj_2] = compute_obj(K, model, para)
    ...
    obj_1 = para.C * sum(xi) + 0.5* quad_form(model.alp, model.support_v*model.support_v');
